library(deSolve) 
library(signal)




##### model ######

seir_model20 = function (current_timepoint, state_values, parameters)
{
  
  # naredimo lokalne spremenljivke
  S = state_values [1]          # susceptibles
  E = state_values [2]          # exposed
  I = state_values [3]          # infectious
  
  Mild   = state_values [4]       # Recovering mild
  Hosp   = state_values [5]       # recovering hospital
  ICU    = state_values [6]        # recovering ICU
  Dead_s = state_values [7]     # dying symp
  Dead_a = state_values [8]     # dying asymp 
  
  Rmild = state_values [9]        # recovered mild
  Rhosp = state_values [10]       # recovered hospital
  Ricu = state_values [11]        # recovered ICU
  Rdead_s = state_values [12]     # dead
  Rdead_a = state_values [13]     # dead
  
  #recovered all together Rmild + Rhosp + Ricu
  
  # parametri
  Bt = parameters[["Bfun"]]  # Bt je funkcija časa
  
  beta  = Bt[current_timepoint]
  a     = 1/parameters[["D_incubation"]]
  g     = 1/parameters[["D_infectious"]]
  
  p_mild = parameters[["p_mild"]]
  gM     = 1/parameters[["D_recovery_mild"]]
  
  p_hosp = parameters[["p_hosp"]]
  gHOSP  = 1/parameters[["D_recovery_hosp"]] 
  
  p_icu  = parameters[["p_ICU"]]
  gICU   = 1/parameters[["D_recovery_ICU"]]
  
  p_fatal_a = parameters[["p_fatal_a"]]
  p_fatal_s = parameters[["p_fatal_s"]]
  gF        = 1/parameters[["D_death"]]
  
  
  with ( 
    as.list (parameters),     
    {
      # diferencialne enačbe
      dS = (-beta * I ) * S
      dE = (beta * I) * S - (a * E)
      dI = (a * E) - (g * I)
      
      dMild       =   p_mild * g * I   - gM * Mild
      dHosp       =   p_hosp * g * I   - gHOSP * Hosp   
      dICU        =   p_hosp * p_icu * g * I   - gICU * ICU
      dDead_s     =   p_fatal_s * p_hosp * p_icu * g * I   - gICU * Dead_s
      dDead_a     =   p_fatal_a * g * I    - gF * Dead_a
      
      dRmild      =   gM * Mild
      dRhosp      =   gHOSP * Hosp
      dRicu       =   gICU  * ICU
      dRdead_s    =   gICU * Dead_s
      dRdead_a    =   gF * Dead_a
      
      
      
      # rezultati
      results = c (dS, dE, dI, dMild, dHosp, dICU, dDead_s, dDead_a, dRmild, dRhosp, dRicu, dRdead_s, dRdead_a)
      list (results)
    }
  )
}

####### fiksni parametri modela ######

fixed_model_parameters = function() {
  
  D_incubation      = 5.2
  D_infectious      = 2.9
  D_recovery_mild   = 12  
  D_recovery_hosp   = 12
  D_recovery_ICU    = 14
  D_death           = 14
  
  
  duration      = 7*12*1e10
  
  p_hosp        =  0.6 / 100        # delež hospitaliziranih
  p_ICU         = 37 / 100          # delež ICU od hospitaliziranih
  p_fatal_s     = 10 / 100          # delež umrlih od ICU
  p_fatal_a     = 0.22 / 100        # delež smrti asimptomatsko
  p_mild        = 1 - p_hosp - p_fatal_a
  
  
  
  parameter_list = list()
  parameter_list$D_incubation = D_incubation 
  parameter_list$D_infectious = D_infectious 
  parameter_list$D_recovery_mild = D_recovery_mild
  parameter_list$D_recovery_hosp = D_recovery_hosp 
  parameter_list$D_recovery_ICU = D_recovery_ICU                        
  parameter_list$D_death = D_death
  parameter_list$p_hosp = p_hosp 
  parameter_list$p_fatal_s = p_fatal_s 
  parameter_list$p_fatal_a = p_fatal_a 
  parameter_list$p_mild = p_mild 
  parameter_list$p_ICU = p_ICU
  parameter_list$duration=duration
  
  return(parameter_list)
  
}

######### funkcija Bt s filtri ########

make_filter_bank = function(n, win_len = 10, frame_skip = 5)
{
  
  num_filt = floor(n / frame_skip) 
  
  tab_filt = matrix(nrow=num_filt+1, ncol = n)
  
  # na zacetku imamo polovicni filter
  filt1 = triang(win_len)
  #filt1 = rep(1, win_len)
  time_filter = rep(0, n)
  time_filter[seq(1, frame_skip)] =  filt1[seq(frame_skip+1,win_len)]
  tab_filt[1,] = time_filter
  
  
  # v sredini
  for (i in seq(1,num_filt-1)) {
    time_filter = rep(0, n)
    #print(seq(1, win_len)+frame_skip*(i-1))
    time_filter[seq(1, win_len)+frame_skip*(i-1)] =  triang(win_len)
    #time_filter[seq(1, win_len)+frame_skip*(i-1)] =  rep(1, win_len)
    tab_filt[i+1,] = time_filter
  }
  
  
  # na koncu imamo polovicni filter
  filt1 = triang(win_len)
  #filt1 = rep(1, win_len)
  time_filter = rep(0, n)
  time_filter[seq(1, frame_skip)+frame_skip*(num_filt-1)] =  filt1[seq(1,frame_skip)]
  tab_filt[num_filt+1,] = time_filter
  
  return(tab_filt)
  
}

### funkcija Bt #####

Bt = function(end_time, Bw, win_len = 10, frame_skip = 5)
{
  a = make_filter_bank(end_time, win_len, frame_skip)
  
  n_bw = length(Bw)
  n_filt = nrow(a)
  if (n_bw < n_filt) {
    repn = n_filt - n_bw
    Bw = c(Bw, rep(Bw[n_bw], repn))
  }
  
  if (n_bw > n_filt) {
    Bw = Bw[1:n_filt]
    
  }
  
  
  B = rowSums(t(a) %*% diag(Bw))
  
  return(B)
  
}



### funkcija za izračun modela s parametri ###

model_seir = function(Bfun){
  
  
  parameter_list = fixed_model_parameters()
  
  parameter_list$Bfun = Bfun 
  
  
  # Začetne vrednosti
  N = 2000000            # populacija
  X = 20                 # začetno okuženi (20, da ne začnemo na začetku)
  W = N-X                # število dovzetnih
  Z = 0                  # število izpostavljenih
  
  
  
  initial_values = c (S = W/N, 
                      E = Z/N, 
                      I = X/W, 
                      Mild = 0,
                      Hosp = 0,
                      ICU = 0,
                      Dead_s = 0,
                      Dead_a = 0,
                      Rmild = 0,
                      Rhosp = 0,
                      Ricu = 0,
                      Rdead_s = 0,
                      Rdead_a = 0)
  
  
  
  timepoints = seq (1, 120, by=1)
  
  
  # Simulate SEIR model
  out_seir = lsoda (initial_values, timepoints, seir_model20, parameter_list)
  out_seir[,2:14] = round(out_seir[,2:14]*N, digits = 0)
  
  output = data.frame(out_seir)
  
  return(output)
  
}



##### izračun modela z dejanskimi podatki #######


izracun_modela_dejanski = function(Bfun, sidat) {
  
  output = model_seir(Bfun)
  
  org_st = output$Hosp
  premk_st = output$Hosp
  for (j in 1:length(org_st)) {
    if (j>4)
      premk_st[j-4] = org_st[j] 
  }
  output$Hosp = premk_st
  
  pdat = data.frame(skupine=c(rep("hospitalizirani", nrow(output)), 
                              rep("ICU", nrow(output)), 
                              rep("kumulativno umrli", nrow(output)) 
                              ), 
                    "stevilo"=c((output$Hosp+output$ICU), 
                                output$ICU,
                                output$Rdead_s + output$Rdead_a
                                ),
                    dnevi = c(output$time, 
                              output$time,
                              output$time
                              )
                    )
  
  
  prva_smrt = head(output$time[(output$Rdead_s + output$Rdead_a) == 1], n=1)
  
  datum_smrti = "2020-03-14"
  pdat$datum = as.Date(datum_smrti) + pdat$dnevi - prva_smrt 
  
  
  # Zložimo podatke, dejanske in simulacijo
  n = sum(pdat$skupine=="hospitalizirani")
  
  pdat$dejansko = rep(NA, 3*n)
  
  n_dej = nrow(sidat)
  for (i in seq(1,n_dej,1)) {
    
    datum = sidat$date[i] 
    
    
    mask = (pdat$skupine=="hospitalizirani" & pdat$datum == datum)
    pdat$dejansko[mask] = sidat$state.in_hospital[sidat$date == datum]
    
    mask = (pdat$skupine=="kumulativno umrli" & pdat$datum == datum)
    pdat$dejansko[mask] = sidat$state.deceased.todate[sidat$date == datum]
    
    mask = (pdat$skupine=="ICU" & pdat$datum == datum)
    pdat$dejansko[mask] = sidat$state.icu[sidat$date == datum]
    
  }
  
  
  return(pdat)
  
}



